...
export default function LoginPage() {
  ...
  const [form, setForm] = useState({ email: 'premium@partybox.com', password: 'password123' })
  ...
  return (
    <div style={{
      ...
      <div style={{ position: 'absolute', filter: 'blur(80px)', top: -100, left: -100, width: 400, height: 400, background: '#FF3366', opacity: 0.15, borderRadius: '50%' }} />
      <div style={{ position: 'absolute', filter: 'blur(80px)', bottom: -100, right: -100, width: 500, height: 500, background: '#FF9933', opacity: 0.15, borderRadius: '50%' }} />
      ...
      <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} style={{
        ...
        boxShadow: '0 20px 60px rgba(255,51,102,0.15)', width: '100%', maxWidth: 440,
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 24 }}>
          ...
          <span style={{ fontWeight: 900, fontSize: 24, letterSpacing: '-0.03em', color: '#FF3366' }}>Glyka PartyBox</span>
          <span style={{ fontSize: 12, fontWeight: 600, color: '#64748b', marginLeft: 8 }}></span>
        </div>

        <h1 style={{ fontSize: 24, fontWeight: 900, marginBottom: 4, letterSpacing: '-0.03em', color: '#18181B' }}>{t('login_title')}</h1>
        <p style={{ color: '#71717A', fontSize: 14, marginBottom: 24, fontWeight: 500 }}>{t('login_subtitle')}</p>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div>
            <label style={{ fontSize: 13, fontWeight: 700, display: 'block', marginBottom: 4, color: '#3F3F46' }}>{t('login_email')}</label>
            <input value={form.email} onChange={e => setForm(p => ({ ...p, email: e.target.value }))}
              type="email" placeholder={t('login_email_placeholder')}
          ...
          <div>
            ...
            <div style={{ position: 'relative' }}>
              <input value={form.password} onChange={e => setForm(p => ({ ...p, password: e.target.value }))}
                type={show ? 'text' : 'password'} placeholder={t('login_password_placeholder')}
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
            <Link href="#" style={{ fontSize: 13, color: '#FF3366', fontWeight: 700, textDecoration: 'none' }}>{t('login_forgot')}</Link>
          </div>
        </div>
        ...
        <p style={{ textAlign: 'center', marginTop: 16, fontSize: 13, color: '#71717A', fontWeight: 500 }}>
          {t('login_no_account')} {' '}
          <Link href="/register" style={{ color: '#FF3366', fontWeight: 800, textDecoration: 'none' }}>{t('login_register_link')}</Link>
        </p>
